...
& dotnet restore (Join-Path $repo 'LicenseScope.sln') --verbosity minimal; if($LASTEXITCODE){throw 'restore failed'}
$testStatus=if($SkipTests){'SKIPPED'}else{'STANDARD_ANYCPU_PASS; ARCHITECTURE_SPECIFIC_BLOCKED_BY_TESTHOST'}
if(-not $SkipTests){& dotnet test (Join-Path $repo 'LicenseScope.sln') -c $Configuration --logger 'console;verbosity=minimal';if($LASTEXITCODE){throw 'standard tests failed'}}
$results=@()
foreach($platform in $Platforms){
  ...
  & dotnet clean (Join-Path $repo 'LicenseScope.sln') -c $Configuration -p:Platform=$platform -p:PlatformTarget=$target --verbosity quiet; if($LASTEXITCODE){throw "clean failed: $platform"}
  & dotnet build (Join-Path $repo 'LicenseScope.sln') -c $Configuration -p:Platform=$platform -p:PlatformTarget=$target --no-restore --verbosity minimal; if($LASTEXITCODE){throw "build failed: $platform"}
  $id=if($platform -eq 'x86'){'licensescope-net48-x86'}elseif($platform -eq 'x64'){'licensescope-net48-x64'}else{'licensescope-net48-anycpu'}
  $dest=Join-Path $out $id;[IO.Directory]::CreateDirectory($dest)|Out-Null
  foreach($source in @((Join-Path $repo "LicenseScope.App\bin\$Configuration\net48"),(Join-Path $repo "src\LicenseScope.Cli\bin\$Configuration\net48"))){Get-ChildItem -LiteralPath $source -File|Copy-Item -Destination $dest}
  $exe=Join-Path $dest 'LicenseScope.Cli.exe';$pe=& (Join-Path $PSScriptRoot 'Get-PEArchitecture.ps1') -Path $exe
