...
$ErrorActionPreference='Stop';$repo=Split-Path $PSScriptRoot -Parent;$sln=Join-Path $repo 'WinLic.sln'
...
$files=@(Get-ChildItem (Join-Path $repo 'build') -Filter *.ps1)+@(Get-ChildItem (Join-Path $repo 'installer\tests') -Filter *.ps1)+@(Get-ChildItem (Join-Path $repo 'tools\compatibility') -Filter *.ps1)+@(Get-Item (Join-Path $repo 'WinLicPS\WinLicManager.ps1'))
