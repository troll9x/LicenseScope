  ...
  #define OutputBaseFilename "LicenseScope-Setup"
#endif
...
AppId={{98FB3801-EC81-49A7-987A-7742D6D01687}
#else
AppId={{#ProductAppId}
#endif
AppName={#ProductName}
...
SetupIconFile=..\LicenseScope.App\licensescope.ico
...
[Icons]
Name: "{group}\License Scope"; Filename: "{app}\{#ProductExe}"; WorkingDir: "{userdocs}"
Name: "{group}\License Scope CLI"; Filename: "{cmd}"; Parameters: "/k &quot;&quot;{app}\{#CliExe}&quot; --help&quot;"; WorkingDir: "{userdocs}"
Name: "{autodesktop}\License Scope"; Filename: "{app}\{#ProductExe}"; Tasks: desktopicon; WorkingDir: "{userdocs}"

[Run]
Filename: "{app}\{#ProductExe}"; Description: "Launch License Scope"; Flags: nowait postinstall skipifsilent runasoriginaluser
...
#ifdef TestInstallMode
  UninstallKey = 'Software\Microsoft\Windows\CurrentVersion\Uninstall\{98FB3801-EC81-49A7-987A-7742D6D01687}_is1';
#else
...
begin
  if UseX64Payload then Result := ExpandConstant('{autopf64}\LicenseScope')
  else Result := ExpandConstant('{autopf32}\LicenseScope');
end;
...
begin
  ...
  if not Exec(ExpandConstant('{tmp}\{#ExtractFileName(PrerequisitePath)}'),
    '/q /norestart /ChainingPackage LicenseScope', '', SW_HIDE, ewWaitUntilTerminated, Code) then begin
