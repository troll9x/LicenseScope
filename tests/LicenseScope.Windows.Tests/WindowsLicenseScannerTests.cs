...
namespace LicenseScope.Windows.Tests
{
    ...
    public sealed class WindowsLicenseScannerTests
    {
        ...
        [TestMethod]
        public async Task PreservesMatchingFullKeyForExplicitUiRevealOnly()
        {
            var evidence = Evidence(Product(1, "RETAIL", "3V66T"));
            evidence.BackupProductKey = "AAAAA-BBBBB-CCCCC-DDDDD-3V66T";
            var result = (await Create(evidence).ScanAsync(Context(), CancellationToken.None))[0];
            Assert.AreEqual("AAAAA-BBBBB-CCCCC-DDDDD-3V66T", result.FullProductKey);
            Assert.AreEqual("XXXXX-XXXXX-XXXXX-XXXXX-3V66T", result.PartialProductKey);
            Assert.IsFalse(result.Evidence.Any(e => e.Value.Contains("AAAAA-BBBBB")));
        }
