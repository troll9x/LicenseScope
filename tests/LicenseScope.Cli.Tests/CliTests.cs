...
namespace LicenseScope.Cli.Tests
{
    [TestClass] public sealed class CliTests
    {
        ...
        [TestMethod] public async Task ConsoleUsesStructuredTraceVerdict(){var audit=Clean();audit.CrackTraceAnalysis=new CrackTraceAnalysisResult{ActivationState=WindowsActivationState.Activated,TraceVerdict=CrackTraceVerdict.Inconclusive,ProvenanceVerdict=LicenseProvenanceVerdict.Unverified,VerdictSummary="KHÔNG THỂ KẾT LUẬN",DetectionCoverage=new[]{new DetectionCoverageItem{Id="current",DisplayName="Current licensing state",Status=DetectionCoverageStatus.Checked}}};var console=new BufferConsole();var sanitizer=new AuditResultSanitizer();var app=new CliApplication(new FakeAudit(audit),new IAuditReportWriter[]{new JsonAuditReportWriter(sanitizer)},console);Assert.AreEqual(0,await app.RunAsync(new[]{"audit","--all"},CancellationToken.None));StringAssert.Contains(string.Join("\n",console.Lines),"TraceVerdict: INCONCLUSIVE");}
