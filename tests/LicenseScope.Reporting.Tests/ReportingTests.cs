...
namespace LicenseScope.Reporting.Tests
{
    ...
    {
        [TestMethod] public void SanitizerDoesNotMutateAndMasksSensitiveData(){var source=Sample("AAAAA-BBBBB-CCCCC-DDDDD-ABCDE","person@example.com C:\\Users\\alice\\x");source.Products[0].FullProductKey="AAAAA-BBBBB-CCCCC-DDDDD-ABCDE";var snap=new AuditResultSanitizer().CreateReportSnapshot(source,new ReportWriteOptions());Assert.AreEqual("AAAAA-BBBBB-CCCCC-DDDDD-ABCDE",source.Products[0].FullProductKey);Assert.AreEqual("XXXXX-XXXXX-XXXXX-XXXXX-ABCDE",snap.Products[0].PartialProductKey);Assert.AreEqual(string.Empty,snap.Products[0].FullProductKey);Assert.IsFalse(snap.Products[0].Warnings[0].Contains("person@example.com"));StringAssert.Contains(snap.Products[0].Warnings[0],"<USER>");Assert.AreEqual(string.Empty,snap.System.MachineName);}
        ...
        [TestMethod] public void JsonNeverExportsFullProductKey(){var a=Sample("3V66T","");a.Products[0].FullProductKey="AAAAA-BBBBB-CCCCC-DDDDD-3V66T";var text=Write(new JsonAuditReportWriter(new AuditResultSanitizer()),"json",a);Assert.IsFalse(text.Contains("AAAAA-BBBBB"));Assert.IsFalse(text.Contains("fullProductKey"));StringAssert.Contains(text,"\"partialProductKey\":\"3V66T\"");}
