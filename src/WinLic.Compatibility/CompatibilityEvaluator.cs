...
namespace WinLic.Compatibility
{
    public sealed class CompatibilityEvaluator : ICompatibilityEvaluator
    {
        ...
        {
            if (environment.NativeOsArchitecture == CpuArchitecture.X86) return "winlic-net48-x86";
            if (environment.NativeOsArchitecture == CpuArchitecture.X64) return "winlic-net48-x64";
            if (environment.NativeOsArchitecture == CpuArchitecture.Arm64 && environment.WindowsVersion.Build < 22000) return "winlic-net48-x86";
            if (environment.NativeOsArchitecture == CpuArchitecture.Arm64) return "winlic-net48-x64";
            return string.Empty;
        }
    }
    ...
    {
        public static PayloadDescriptor Describe(RuntimeEnvironmentInfo environment) => new PayloadDescriptor { Id = environment.ProcessArchitecture == CpuArchitecture.X86 ? "winlic-net48-x86" : environment.ProcessArchitecture == CpuArchitecture.X64 ? "winlic-net48-x64" : "winlic-net48-anycpu", TargetFramework = "net48", ProcessArchitecture = environment.ProcessArchitecture, RequiredFrameworkVersion = new Version(4, 8), BuildVerified = true, RuntimeVerified = environment.WindowsVersion.Major == 10 && environment.WindowsVersion.Build == 19045 && environment.NativeOsArchitecture == CpuArchitecture.X64 };
    }
}
