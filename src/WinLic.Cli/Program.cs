...
namespace WinLic.Cli
{
    ...
    public sealed class CliApplication
    {
        ...
        {
            try{var parsed=Parse(args??Array.Empty<string>());if(parsed.Help){PrintHelp();return 0;}if(parsed.Version){_console.WriteLine(typeof(CliApplication).Assembly.GetName().Version?.ToString()??"1.0.0.0");return 0;}if(!parsed.Valid){_console.WriteLine("Error: "+parsed.Error);_console.WriteLine("Use --help for usage.");return 4;}var result=await _audit.RunAllAsync(token).ConfigureAwait(false);var consoleSnapshot=new AuditResultSanitizer().CreateReportSnapshot(result,new ReportWriteOptions{IncludeEvidence=false,IncludeWarnings=false});if(!parsed.Quiet){foreach(var p in consoleSnapshot.Products)_console.WriteLine(p.ScannerId+" | "+p.ProductName+" | "+p.Status+" | "+p.LicenseType);}
